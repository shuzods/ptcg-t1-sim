...
def front_pick(view8, c1, c2, c3, c4):
    ...
    規則2は 2026-09-18その4 に「常に条件2」へ変更した。条件1未達=先1が崩れた配りでは、
    先2までにエネ3枚を要求するガルーラの線がほぼ成立しない(この領域での寄与は
    ガルーラの線 1.13pt に対し条件2 3.79pt、完全情報上限 4.19pt)。
    30万試行で比較した5候補のうち、8枚の見え方で分岐させるどの規則よりも
    「常に条件2」が良かった:
      現行(ファイアロー/シアノ)3.13 / 常に条件2 **3.79** /
      アカマツ見えたらガルーラ 3.68 / 同(ハイパー除外)3.55 / つけかえ+草も見る 3.60
    実戦的にも「先1でガルーラをまともに立てられなかったらファイアローに切り替える」
    という判断に対応する(ファイアローは無色2でよく、エキサイトダイブで無償にベンチへ出せる)。
    view8 は規則2が参照しなくなったが、将来の精緻化のため引数に残す。"""
    gar = c3 or c4
    if c1:
        return c2 or gar   # 規則1: 先1の動きが共通→選択を先2に持ち越す
    return c2             # 規則2: 先1が崩れた配りはファイアローの線に切り替える
